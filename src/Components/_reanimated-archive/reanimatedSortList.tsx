...
> {
      ...
      _rowRenderer(type, data, index, _, nope) {
            ...
            return (
                  ...
                  >
                        ...
                        <Text style={{ fontSize: 18, textAlign: 'center', flex: 1 }}>{data}</Text>
                  </View>
            );
      }
      ...
      yToIndex = (y: number) =>
            Math.min(
                  ...
                  Math.max(0, Math.floor((y + this.scrollOffset - this.topOffset) / this.rowHeight))
            );

      reset = () => {
            this.setState({
                  dataProvider: this.state.dataProvider.cloneWithRows(this.state.dataProvider.getAllData()),
                  dragging: false,
                  draggingIdx: -1
            });
            ...
            if (this.currIdx !== newIdx) {
                  this.setState({
                        dataProvider: this.state.dataProvider.cloneWithRows(
                              immutableMove(this.state.dataProvider.getAllData(), this.currIdx, newIdx)
                        ),
                        draggingIdx: newIdx
                  });
                  this.currIdx = newIdx;
      ...
      moveList = amount => {
            ...
            this.list.current.scrollToOffset(this.scrollOffset + amount, this.scrollOffset + amount, false);
      ...
      render() {
            ...
            return (
                  <SafeAreaView style={{ flex: 1 }}>
                        <Animated.Code>
                              {() => cond(eq(this.gestureState, State.BEGAN), call([this.offY], this.start))}
                        </Animated.Code>
                        <Animated.Code>
                              {() =>
                                    cond(
                        ...
                        <Animated.Code>
                              {() => cond(eq(this.gestureState, State.ACTIVE), call([this.offY], this.move))}
                        </Animated.Code>
                        {dragging ? (
                              <Animated.View
                                    style={{
                              ...
                              >
                                    {this._rowRenderer(-1, dataProvider.getDataForIndex(draggingIdx), -1, -1, true)}
                              </Animated.View>
                        ) : null}
                        <View
